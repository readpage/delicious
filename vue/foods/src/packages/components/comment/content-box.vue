    ...
    <div class="content-box">
      <div class="user-box">
        <a href="/user/1556564194374926" target="_blank" rel="" class="username">
          ...
          <span blank="true" class="rank">
            <u-icon name="iconLv1" size="24"></u-icon>
          </span> 
        </a>
        <!-- <span class="author-badge-text">（作者）</span> -->
        <time datetime="1646629604000" title="Mon Mar 07 2022 13:06:44 GMT+0800 (中国标准时间)" class="time">{{data.createTime}}</time>
      </div>
      <u-fold unfold>
      ...
      <div class="action-box user-select">
        ...
        <div ref="btnRef" class="item" :class="{active: active}"  @click="reply">
          <u-icon name="iconcomment"></u-icon>
          <span>{{active? "取消回复": '回复'}}</span>
        </div>
      </div>
      <div v-if="active">
        <CommentBox :parentId="parentId" ref="commentRef" :placeholder="`回复${data.username}...`" content-btn="发布" style="margin-top: 12px;" @hide="hide" />
      </div>
      <slot></slot>
    </div>
  </div>
